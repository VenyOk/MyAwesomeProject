"""Dev server: run the Second Brain web UI with fake LLM/embedder/index.

No GPU, no model download, no faiss/sentence-transformers required. Intended for
UI preview and Playwright driving only — answers are canned ("Ответ от FakeLLM").
Memory and slash commands work for real (SQLite + brute-force index).
